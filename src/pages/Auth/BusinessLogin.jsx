    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Admin, Staff and Suppliers sign in here</p>
        </div>

        {notification && (
          <div className={`mb-6 p-4 rounded-lg flex items-center gap-3 ${notification.type === 'success' ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`}>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="px-8 py-6 border-b border-gray-100">
            ...
            <p className="text-sm text-gray-600 mt-1">Use credentials provided by the Admin</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <button type="submit" disabled={isLoading} className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gray-900 hover:bg-black focus:ring-gray-700'} text-white shadow-lg`}>{isLoading ? (<><Loader2 className="h-5 w-5 animate-spin" /> Signing In...</>) : (<><LogIn className="h-5 w-5" /> Sign In</>)}</button>
          </form>

          <div className="px-8 py-6 bg-gray-50 border-t border-gray-100 text-sm text-gray-600">
            Are you a customer? <Link to="/login" className="text-blue-600 hover:text-blue-500">Go to Customer Login</Link>
          </div>
        </div>
