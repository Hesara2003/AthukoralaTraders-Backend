...
export default function PurchaseOrderForm() {
  ...
  if (loading) return <AdminLayout title={editing ? 'Edit Purchase Order' : 'New Purchase Order'}><div className="text-gray-600">Loading...</div></AdminLayout>;
  if (error) return <AdminLayout title="Purchase Order"><div className="text-red-600">{error}</div></AdminLayout>;

  return (
    <AdminLayout title={editing ? 'Edit Purchase Order' : 'New Purchase Order'}>
      {!canEdit && (
      ...
      <form className="space-y-6" onSubmit={save}>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" className="px-4 py-2 bg-gray-100 rounded" onClick={() => navigate('/admin/purchase-orders')}>Cancel</button>
        </div>
      </form>
    </AdminLayout>
  );
