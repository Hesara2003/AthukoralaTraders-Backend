  ...
  useEffect(() => {
    ...
    async function fetchProduct() {
      try {
        ...
        setLoading(true);
        // Use customer-facing endpoint that returns promotion-aware ProductDetailDTO
      ...
      <div className="relative">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-2 text-sm mb-8">
            <Link to="/" className="px-3 py-1.5 rounded-full text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200">Home</Link>
            <ChevronRight className="h-4 w-4 text-gray-400" />
            <Link to="/products" className="px-3 py-1.5 rounded-full text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200">Shop</Link>
            {product.category && (
              <>
                <ChevronRight className="h-4 w-4 text-gray-400" />
                <button
                  onClick={() => navigate(`/products?category=${encodeURIComponent(product.category)}`)}
                  className="px-3 py-1.5 rounded-full text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
                >
                  {product.category}
                </button>
              </>
            ...
            <ChevronRight className="h-4 w-4 text-gray-400" />
            <span className="px-3 py-1.5 rounded-full text-gray-900 bg-gray-100 font-medium line-clamp-1">{product.name}</span>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Premium Product Gallery */}
            <div className="space-y-6">
              <div className="relative aspect-square bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border-0 shadow-2xl shadow-gray-900/10 overflow-hidden group">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent leading-tight">{product.name}</h1>
                    <div className="flex items-center gap-3">
                      <span className="px-3 py-1 bg-gradient-to-r from-blue-100 to-blue-50 text-blue-800 text-sm font-medium rounded-full border border-blue-200">{product.category}</span>
                      {product.brand && (
                        <span className="px-3 py-1 bg-gradient-to-r from-purple-100 to-purple-50 text-purple-800 text-sm font-medium rounded-full border border-purple-200">by {product.brand}</span>
                      )}
                    </div>
                  </div>
                  {product.sku && (
                ...
                <div className="flex items-center gap-6">
                  {product.discountPercent != null && product.discountedPrice != null && product.discountedPrice < product.price ? (
                    <div className="flex items-baseline gap-3">
                      <div className="text-5xl font-extrabold bg-gradient-to-r from-green-600 to-emerald-500 bg-clip-text text-transparent">${(product.discountedPrice).toFixed(2)}</div>
                      <div className="text-2xl font-semibold text-gray-400 line-through">${(product.price).toFixed(2)}</div>
                      <span className="ml-1 inline-flex items-center text-sm font-bold text-white bg-red-500 px-3 py-1 rounded-full shadow">-{product.discountPercent}%</span>
                    </div>
                  ) : (
                    <div className="text-5xl font-bold bg-gradient-to-r from-green-600 to-green-500 bg-clip-text text-transparent">${parseFloat(product.price).toFixed(2)}</div>
                  )}
                  <div className={`flex items-center gap-2 px-4 py-2 rounded-full border-2 ${stockStatus.color} shadow-sm`}>
              ...
              <div className="bg-gradient-to-br from-blue-50/50 to-white p-6 rounded-2xl border border-blue-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {product.brand && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Brand</span>
                      <span className="text-gray-600">{product.brand}</span>
                    </div>
                  )}
                  {product.model && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Model</span>
                      <span className="text-gray-600">{product.model}</span>
                    </div>
                  )}
                  {product.color && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Color</span>
                      <span className="text-gray-600">{product.color}</span>
                    </div>
                  )}
                  {product.material && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Material</span>
                      <span className="text-gray-600">{product.material}</span>
                    </div>
                  )}
                  {product.weight && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Weight</span>
                      <span className="text-gray-600">{product.weight}</span>
                    </div>
                  )}
                  {product.dimensions && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Dimensions</span>
                      <span className="text-gray-600">{product.dimensions}</span>
                    </div>
                  )}
                  <div className="bg-white p-3 rounded-lg border border-gray-100">
                    <span className="font-semibold text-gray-800 block">Stock</span>
                    <span className="text-gray-600">{product.stock} units</span>
                  </div>
                  {product.warranty && (
                    <div className="bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Warranty</span>
                  ...
                  {product.specifications && (
                    <div className="md:col-span-2 bg-white p-3 rounded-lg border border-gray-100">
                      <span className="font-semibold text-gray-800 block">Additional Details</span>
                      <span className="text-gray-600">{product.specifications}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {product.stock > 0 && (
                <div className="bg-gradient-to-br from-white to-gray-50 p-6 rounded-2xl border border-gray-200 shadow-lg space-y-6">
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden bg-white">
                        <button
                          ...
                          className="w-12 h-12 flex items-center justify-center hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 font-bold text-lg"
                        >
                          −
                        </button>
                        <input
                          ...
                          className="w-20 text-center border-0 py-3 font-bold text-lg focus:ring-0"
                        />
                        <button
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-8 py-4 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-[1.02] disabled:hover:scale-100 flex items-center justify-center gap-3"
                    >
                      {addingToCart ? (
                        <>
                    ...
                    <button className="w-16 h-16 border-2 border-gray-200 rounded-xl flex items-center justify-center hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-all duration-200 group">
                      <Heart className="h-6 w-6 text-gray-400 group-hover:text-red-600" />
                    </button>
                    <button className="w-16 h-16 border-2 border-gray-200 rounded-xl flex items-center justify-center hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 transition-all duration-200 group">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 rounded-xl border border-blue-200 flex items-center gap-3 group hover:shadow-md transition-all duration-200">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">On all orders</p>
                  </div>
                </div>
                <div className="bg-gradient-to-br from-green-50 to-green-100/50 p-4 rounded-xl border border-green-200 flex items-center gap-3 group hover:shadow-md transition-all duration-200">
                  ...
                  <div>
                    <p className="font-semibold text-green-900">{product.warranty || '1 Year Warranty'}</p>
                    <p className="text-sm text-green-700">Protected</p>
                  </div>
                </div>
                <div className="bg-gradient-to-br from-orange-50 to-orange-100/50 p-4 rounded-xl border border-orange-200 flex items-center gap-3 group hover:shadow-md transition-all duration-200">
                  <div className="w-12 h-12 bg-orange-600 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                    <RotateCcw className="h-6 w-6 text-white" />
                  </div>
                  <div>
                    <p className="font-semibold text-orange-900">30-Day Returns</p>
