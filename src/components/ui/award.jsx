...
export function Awards({
  ...
  if (variant === "stamp") {
    ...
    const createTextPath = (radius, id) => {
      const centerX = 96
      const centerY = 96
        ...
        <div className="relative z-10 text-center">
          {showIcon && (
            <div className="mb-1 flex justify-center text-center text-2xl">
              {customIcon || <Star className="text-primary fill-primary" />}
            </div>
          )}
          {recipient && (
      ...
      <div className={cn("relative mx-auto w-full max-w-xl", className)}>
        <div className="relative flex min-h-[320px] items-center justify-center p-8">
          <div className="relative z-10 text-center">
            ...
            <div
              className={cn(
                "mx-auto mb-4 inline-block rounded-md px-6 py-1.5 text-sm font-bold tracking-wider text-white",
                `bg-gradient-to-r ${levelColors[level]}`
              )}
            >
              {level.toUpperCase()}
            ...
            {/* Main Title */}
            <h1 className={cn("mb-2 text-5xl font-black tracking-tight")}>{title}</h1>

            {/* Decorative Line */}
            <div className="bg-foreground mx-auto my-4 h-[2px] w-48"></div>

            {/* Subtitle */}
            <h2 className={cn("mb-6 text-lg font-light")}>{subtitle}</h2>

            {/* Recipient */}
            {recipient && (
              <p className={cn("mb-2 text-base italic opacity-70")}>{recipient}</p>
            )}

            {/* Date */}
            {date && <div className={cn("text-xl font-bold")}>{date}</div>}
          </div>
        </div>
      </div>
    )
